    ...
    <div className="flex h-full flex-nowrap items-center justify-between gap-2.5">
      {items.map((item) => (
        <button
          type="button"
          key={item.id}
          aria-label={`${item.label} — ${item.version}`}
          className="group relative inline-flex h-7 w-7 cursor-default items-center justify-center rounded-full border border-[var(--color-border-soft)] bg-[var(--button-secondary-bg)] transition-[background-color,border-color,translate] duration-200 ease-out hover:border-[var(--color-border-soft)] hover:bg-surface hover:-translate-y-0.5 focus-visible:border-[var(--color-border-soft)] focus-visible:bg-surface"
        >
